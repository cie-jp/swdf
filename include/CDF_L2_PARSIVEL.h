...
#ifndef _CDF_L2_PARSIVEL_H_INCLUDE_
#define _CDF_L2_PARSIVEL_H_INCLUDE_
...
extern "C" {
    ...
    typedef struct{                                 //対応データNo.    DEPEND0 DEPEND1 DEPEND2 依存関係                  UNITS            SCALETYP(基本的にlinearで, ダイナミックレンジ(値の最小・最大の幅)が広くグラフが見づらい場合にlogとします. 一般的な例を以下に記載します. )
        ...
        //L2
        CDF__REAL4          R_rain;                  //降雨強度                  (CDF_L2_PARSIVEL__R_rain  )
        CDF__REAL4          R_snow;                  //降雪強度                  (CDF_L2_PARSIVEL__R_snow  )
        CDF__REAL4          Re[32][32];              //レイノルズ数               (CDF_L2_PARSIVEL__Re       )
        CDF__REAL4          Z;                       //レーダ反射因子       (CDF_L2_PARSIVEL__Z       )
        CDF__REAL4          Ze_rain;                 //降雨の等価レーダ反射因子  (CDF_L2_PARSIVEL__Ze_rain   )
        CDF__REAL4          Ze_snow;                 //降雪の等価レーダ反射因子  (CDF_L2_PARSIVEL__Ze_snow   )
        CDF__REAL4          K;                       //降雪種パラメーター          (CDF_L2_PARSIVEL__K)
        CDF__REAL4          E;                       //降雪種パラメーター          (CDF_L2_PARSIVEL__E)
    }CDF_L2_PARSIVEL__DATA;
    
    void CDF_L2_PARSIVEL__Create   (CDFid      *id_ptr,const char *filepath,const char *data_version);

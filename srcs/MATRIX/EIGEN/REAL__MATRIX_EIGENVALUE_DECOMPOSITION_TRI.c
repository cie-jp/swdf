...
void REAL__MATRIX_EIGENVALUE_DECOMPOSITION_TRI(REAL U [],//(out)[n * n] : 直交行列
  ...
  INT   i,j,k;  
  INT   m;
  REAL  reps     = 1.0e-12;//許容相対誤差
  INT   max_iter =   10000;//最大反復回数
  INT   l;                 //ループカウンタ
  REAL  t0_tmp;
  REAL  t1_tmp;
  ...
  for(m = n;m > 1;m--){
    for(l = 0;l < max_iter;l++){
      ...
      t1_tmp = t1[m - 2];      
      // ******************************************************************
      //  5. 右下2x2対称行列の2つの固有値を取得
      // ******************************************************************  
      REAL__MATRIX_EIGENVALUES_2D(&emax,&emin,t0[m - 2],t1[m - 2],t0[m - 1]);
      // ******************************************************************
      //  6. 右下2x2対称行列の非対角成分が十分小さければ減次
      // ******************************************************************  
      if((REAL__MUL(reps,REAL__ABS(emax)) >= REAL__ABS(t1[m - 2])) &&
         (REAL__MUL(reps,REAL__ABS(emin)) >= REAL__ABS(t1[m - 2])) ){
        break;
      }         
      // ******************************************************************
      //  7. QR法のWilkinsonシフト量を固有値から計算
      // ******************************************************************        
      ...
      // 12. 右下成分が収束していれば減次
      // ******************************************************************  
      if(REAL__EQ(t0_tmp,t0[m - 1]) && REAL__EQ(t1_tmp,t1[m - 2])){
        WARNING__SHOW("#1");
        break;
      }
  ...
  // ******************************************************************
  // 13. 固有値を絶対値に関して降順に並び替え(固有ベクトルも対応するように並び替える)
  // ******************************************************************
  for(i = 0;i < n - 1;i++){
    for(j = i + 1;j < n;j++){

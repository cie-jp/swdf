...
//            |    0      0      0      0  ...         0   b0[n - 1] |
// について, その特異値分解
// B = U diag(s) V^{T}を計算する. また, 行列の階数を返す.  
// ここで, sは特異値からなる配列であり, 
//            |  s[0]     0      0      0  ...         0          0  |
//            |    0    s[1]     0      0  ...         .          .  |
//            |    0      0    s[2]     0  ...         .          .  |
...
//
// *******************************************************************
INT  REAL__MATRIX_SVD_BI(REAL s [],//(out)[n]     : 特異値配列
  ...
  // ******************************************************************
  for(i = 0;i < n;i++){
    s[i] = REAL__GE(t0[i],REAL__ZERO()) ? REAL__SQRT(t0[i]) : REAL__ZERO();
  }
  ...
  // ******************************************************************
  if(rank != n){
    REAL__MATRIX_BBT(t0,t1,b0,b1,n);
    REAL__MATRIX_EIGENVALUE_DECOMPOSITION_TRI(U,t0,t1,n);    
  }
  for(i = 0;i < n - 1;i++){
    for(j = 0;j < rank;j++){
